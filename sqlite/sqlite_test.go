...
func TestRepository_UpdateIPRule(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		ipRuleUpdate := &antibrut.IPRuleUpdate{
			Type:   20,
		...
		gotIPRule, err := repo.UpdateIPRule(ctx, existIPRule.ID, ipRuleUpdate)
		require.NoError(t, err)

		require.Equal(t, existIPRule.ID, gotIPRule.ID)
		require.Equal(t, ipRuleUpdate.Type, gotIPRule.Type)
		require.Equal(t, ipRuleUpdate.Subnet, gotIPRule.Subnet)
